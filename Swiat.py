...
class Swiat:
    def __init__(self, szerokosc: int, wysokosc: int):
        ...
        self._plansza: list[list] = [[None] * szerokosc for _ in range(wysokosc)]
        self._gra: list = []

        self._root = tk.Tk()
        self._root.title("Yuriy Dyedyk 201316")

        self._grid_frame = tk.Frame(self._root)
        self._grid_frame.pack()

        self._log = tk.Text(self._root, width=50, height=10)
        self._log.pack()
    ...
    def initialize_grid(self):
        for w in self._grid_frame.winfo_children():
            w.destroy()
        occupied = {(o.x, o.y): o for o in self._gra if o is not None}
    ...
    def add_organism(self, org):
        self._plansza[org.x][org.y] = org
        self._gra.append(org)
    ...
    def _place_random(self, org_id: str):
        for _ in range(self._szerokosc * self._wysokosc * 4):
            x = random.randrange(self._wysokosc)
            y = random.randrange(self._szerokosc)
            if self._plansza[x][y] is None:
                org = self._make(x, y, org_id)
                if org:
                    self.add_organism(org)
                return
    ...
    def wykonajTure(self, keycode=None):
        ...
        for i in range(len(self._gra) - 1, -1, -1):
            org = self._gra[i]
            if org is None:
                continue
            px, py = org.x, org.y
            org.begin_turn()
            org.akcja(self._plansza, self._gra, self._szerokosc, self._wysokosc, keycode)
            nx, ny = org.x, org.y

            if isinstance(org, Roslina) and org.rozsiane:
                self._disperse_plant(org)
            elif self._plansza[nx][ny] is None or (nx == px and ny == py):
                self._plansza[nx][ny] = org
                if (nx, ny) != (px, py):
                    self._plansza[px][py] = None
            elif isinstance(org, Zwierze):
                self._resolve_collision(org, i, px, py)

        for org in self._gra:
            if org is not None:
                org.end_turn()
        self.update_grid()

    def _prepare_turn(self):
        self._gra = [o for row in self._plansza for o in row if o is not None]
        self._gra.sort(key=lambda o: (-o.inicjatywa, -o.wiek))
    ...
    def _resolve_collision(self, attacker, idx: int, px: int, py: int):
        ...
        if isinstance(defender, Zwierze) and attacker.czy_rozmnaza_sie(defender):
            for dx, dy in _DIRECTIONS:
                ...
                if (0 <= nx < self._wysokosc and 0 <= ny < self._szerokosc
                        and self._plansza[nx][ny] is None):
        ...
        if winner is attacker:
            for j, g in enumerate(self._gra):
                if g is defender:
                    self._gra[j] = None
                    break
            self._plansza[px][py] = None
    ...
    def _disperse_plant(self, org):
        ...
        if (0 <= nx < self._wysokosc and 0 <= ny < self._szerokosc
            ...
            if seedling:
                self.add_organism(seedling)
                self.print_log(f"{org.imie} rozsial sie na pole {nx} {ny}")

    def save_world_to_file(self):
        try:
            czlowiek = next((o for o in self._gra if o is not None and isinstance(o, Czlowiek)), None)
            ...
            with open(_SAVE_FILE, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([self._wysokosc, self._szerokosc, c_licznik, c_cooldown, c_wlacz])
                for org in self._gra:
                    if org is not None:
    ...
    def wczytaj(self, file_path: str = _SAVE_FILE):
        try:
            with open(file_path, newline='') as f:
                reader = csv.reader(f)
                header = next(reader)
                self._wysokosc = int(header[0])
                self._szerokosc = int(header[1])
                licznik  = int(header[2])
                cooldown = int(header[3])
                wlacz    = header[4].strip().lower() == 'true'
                self._gra.clear()
                self._plansza = [[None] * self._szerokosc for _ in range(self._wysokosc)]
                for row in reader:
    ...
    def start(self):
        ...
        for key, code in [('<Left>', 37), ('<Up>', 38), ('<Right>', 39), ('<Down>', 40), ('1', 49)]:
            self._root.bind(key, lambda e, c=code: self.wykonajTure(c))
        self._root.mainloop()
